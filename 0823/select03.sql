...
-- 관계연산자
-- : where절에서 크다/작다/같다 등을 지정하는 기호 (부등호)

-- 논리연산자
-- : 관계연산자가 2개 이상 나오는 경우 and, or 등으로 참/거짓을 판별
...
-- 2. 다중 조건 조회:
-- 주소가 '서울'이면서 평균 키가 165 이상인 회원의 정보 조회
-- '디지털' 분류에 속하면서 단가가 150 이상인 구매 내역 조회
...
-- 3. 문자열 검색 조건:
-- 회원 이름에 '소녀'가 포함된 회원 정보 조회
-- '갤럭시'라는 단어가 포함된 제품을 구매한 구매 내역 조회
...
-- 4. 범위 조건 조회:
-- 평균 키가 160 ~ 165인 회원의 정보 조회
-- 단가가 30 이상 100 이하인 구매 내역 조회
...
-- 5. 조건 조합 조회:
-- 주소가 '서울'이면서 연락처 국번(phone1)이 '02'가 아닌 회원의 정보 조회
-- 단가가 50 초과이며, 수량이 3개 이상인 구매 내역 조회
...
-- 프로그램은 unknown(불명확)으로 간주함

-- 연락처 국번(phone1)이 null인 회원의 정보 조회
select *
	from members
    where phone1 is null;

-- 분류(group_name)가 null인 구매 내역 조회
select *
	from buy
...
-- 주소가 '서울'이면서 phone1의 값이 02가 아니거나 null인 행을 모두 조회
select *
	from members
    where addr = '서울' and (phone1 <> '02' or phone1 is null);

-- 7. 날짜 데이터 조건 조회:
-- debut_date의 형식은 DATE 타입: YYYY-MM-DD
...
-- 2015년 1월 1일 이후(>)에 데뷔한 회원의 정보 조회
select *
	from members
