...
-- : 하나의 테이블은 여러 개의 unique 키를 가질 수 있음

-- PK와의 차이점
-- : PK는 null 값 허용 X (not null)
-- : UQ는 null 값 허용 O (null 허용 가능) # null 값은 중복 가능

-- : PK는 테이블 내에서 1번만 지정 가능
-- : UQ는 테이블 내에서 여러 개의 UQ를 가질 수 있음
...
insert into member values('ASP', '에스파', 166, '054');
insert into member values('RED', '레드벨벳', default, default);

select * from member;

-- Null 값 허용
-- : 값을 비워진 채로 둘 것인지 아닌지 지정
-- : PK가 설정된 열에는 null 값이 있을 수 없음
