...
-- 자동을 증가하는 부분은 insert문을 사용할 때 null 값으로 채움
insert into practice2 values (null, '보핍', 25);
insert into practice2 values (null, '슬링키', 22);
insert into practice2 values (null, '티보이', 23);
...
insert into practice2 values (null, '세냐', 28);
...
insert into practice3 values (null, '니키', 23);
insert into practice3 values (null, '앵글러', 26);
insert into practice3 values (null, '데미안', 25);
...
-- 3-1. 기본 형태
-- insert into 테이블명 (열_이름1, 열_이름2, ...)
-- 	select문;

-- 3-2. 주의할 점
-- : select문의 열 개수는 insert할 테이블의 열 개수와 같아야함

-- world 데이터베이트의 city 테이블 개수 조회
-- 열의 개수를 셀 경우 count(*)
-- 데이터베이스명.테이블명: 다른 데이터베이스의 테이블에 접근 가능
select count(*) from world.city;

-- desc 명령: 테이블의 구조를 출력해주는 기능 (describe 약자)
-- descending: 내림차순 정렬 방식(desc)과는 다름
