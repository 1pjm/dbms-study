...
-- Error Code: 1264. Out of range value for column 'tinyint_col'

-- tinyint (1바이트): -128 ~ 127 (256개 표시)
-- 데이터형식 unsigned
...
-- varchar가 char 보다 공간을 효율적으로 운영하지만
-- MySQL 내부적으로 성능(속도)면에서는 char로 설정하는 것이 좋음

-- 전화번호를 문자형으로 지정하는 이유
-- : 정수형으로 전화번호를 지정하는 경우 제일 앞에 사용되는 0이 자동 생략되기 때문
