...
-- if문의 기본 형식
-- : if문은 조건식이 참이면 SQL 문장들을 실행
-- : 그렇지 않으면 그냥 넘어감

-- if <조건식> then
-- 		<sql 문장 작성>
--     	# sql 문장이 두 문장 이상일 경우 begin ~ end로 묶어야함
-- end if
...
begin
	if 100 = 100 then # sql에서는 == 가 아닌 =(등호 1개)를 사용
		select '100은 100과 같다.'; # select 뒤에 따옴표 문자가 바로 오면 print의 기능을 수행
	end if;
...
begin
    ...
    # datediff: 날짜의 차이를 일 단위로 출력
    
    if (days/365) >= 5 then # 5년이 지났다면
		select concat('데뷔한 지', days, '일이 지났습니다. 축하 드립니다.');
	else
		select '데뷔한 지' + days + '일 입니다.';
	end if;
end $$
delimiter ;
