...
-- 2. 문제: 각 학생의 전공에 따라, 그들이 수강한 강좌의 총 학점(total_credits)을 계산하세요. 
...
# group by절을 사용할 때 select절의 모든 열이 포함되어 있어야함
# sum()과 같은 집계 함수를 사용할 경우 예외

-- 3. 문제: 1부터 5까지의 학생ID에 대해 각 학생이 등록한 강좌의 수를 출력하세요.
-- 프로시져 사용

drop procedure if exists getEnrollmentCount;
# 프로시저를 생성하고 호출한 후에는 반드시 삭제하는 것이 좋음
