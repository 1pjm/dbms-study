...
insert into trigger_table values(2, '레드벨벳');
insert into trigger_table values(3, '오마이걸');
...
create trigger myTrigger
	after delete # 이 트리거를 delete문이 발생된 이후에 작동 하라는 의미
    on trigger_table # 이 트리거를 부착할 테이블을 지정
    for each row # 각 행마다 적용시킴을 의미 (트리거에 항상 작성하는 것이 관례)
begin
...
insert into trigger_table values(4, '아이브');
select @msg;

update trigger_table set txt = '레벨' where id = 2;
select @msg;
