...
-- 변경한 사용자, 시간 , 변경 전의 데이터 등을 기록하는 트리거 생성
...
delimiter $$
create trigger singer_updateTrg # 트리거 이름 지정
...
-- OLD 테이블에 update문이 작동되면 이 행에 의해서 업데이트 되기 전까지의 데이터가
-- 백업 테이블(backup_singer)에 입력 -> 원래 데이터가 보존됨

-- curdate(): 현재 날짜
-- current_user(): 현재 작업 중인 사용자를 알려주는 함수
...
update singer
set addr = '영국' where mem_id = 'OMY';

-- singer 테이블에서 멤버수가 7명 이상인 데이터를 삭제
delete from singer where mem_number >= 7;

-- backup_singer 테이블 조회
...
select * from backup_singer;
# truncate로 삭제된 데이터가 저장되어 있지 않음
# truncate table로 삭제할 경우 트리거가 작동되지 X

-- 트리거가 사용하는 임시 테이블

-- 테이블에 insert, update, delete 작업이 수행될 경우 임시로 사용되는 시스템 테이블이 2개
-- : NEW, OLD 테이블
-- : mySQL 내장 테이블

-- 1. NEW 테이블
-- : insert문이 실행될 때 동작
-- : insert문 실행 시 해당 데이터가 NEW 테이블에 저장 되었다가
-- : 사용자가 생성한 테이블로 이동
-- : 자주 사용하지 않음

-- 2. OLD 테이블
-- : after, delete문이 실행될 때 동작
-- : update, delete문 실행 시 해당 데이터가 OLD 테이블에 저장 되었다가
-- : 지정된 동작 수행
-- : 트리거 사용 시 사용함
